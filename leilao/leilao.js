...
jQuery(document).ready(function(){	
	...
	}else if(site_utilizado == "l"){ //VERIFICA SE O SITE É LANCES DE VANTAGENS	
				
		var detalhe = jQuery(".tdhead_main:eq(1)").html().substr(20,5);
	...
	if(site_utilizado == "m" || site_utilizado == "l"){ //VERIFICA SE O SITE É MUKIRANA
	
		jQuery("body").prepend("<div style='position:fixed; z-index:9999; display:table; width:20%; font-size:12px; color:#FFF; top:0; left:40%; background:#8B0000; padding:10px;' align='center'><div class='area_tempo' style='display:inline-block; zoom:1; *display:inline; width:40px;'>Ativo</div><div class='botao_tempo' style='display:inline-block; zoom:1; *display:inline; width:90px; margin-left:20px;'><a href='#topo' style='color:#FFFFFF; text-decoration:none; display:block; padding:2px 10px; border:1px solid #EE0000; background:#CD0000; text-align='center'>Pausar</a></div></div>");
		...
		}else if(site_utilizado == "l"){
			
			iniciarTemporizador = setInterval("temporizador2()",1);
		...
		jQuery(".botao_tempo a").toggle(
			...
			function() {
				...
				jQuery(".area_tempo").html("Pausado");
	...
	jQuery(".iniciar_"+id_produto).toggle(
		...
		function() {
			...
			if(site_utilizado == "m"){ //VERIFICA SE O SITE É MUKIRANA
				...
				intervalo = setInterval("ultimo_lance_l()",1); //EXECUTA A VERIFICAÇÃO
...
function temporizador2(){
	...
	if(jQuery("#modal_user_idle").parent().is(":visible")){
				
		jQuery('#modal_user_idle').dialog('close');
